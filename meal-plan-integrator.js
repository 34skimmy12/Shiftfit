/* ShiftFit Meal Plan Controller
 * Single orchestration point for the Build Your Plan flow.
 * The smart meal engine remains the source-of-truth generator.
 */
(function(){
  "use strict";
  ...
  function addStyles(){
    ...
    s.textContent=`
      ...
      #${PREF_ID} input{width:100%;height:48px;border:1px solid #374164;border-radius:11px;background:#080d1d;color:white;padding:0 12px;font-size:14px;font-weight:700;outline:none;box-sizing:border-box}
  ...
  function generate(plan){
    ...
    try{
      ...
      console.error("ShiftFit smart meal generation failed",error);
      return false;
    }
  }

  window.shiftfitGenerateMealPlan=function(plan){
    return generate(capturePreferences(ensureTargets(plan||getPlan())));
  };
  ...
  function wireGenerate(){
    ...
    function wrappedGeneratePlan(){
      const captured=capturePreferences(getPlan());
      writeJSON("shiftfitPlan",captured);
      let result;
      ...
      setTimeout(()=>window.shiftfitGenerateMealPlan(getPlan()),50);
      return result;
    }
