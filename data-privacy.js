...
(function(){
  ...
  function findProfile(){
    const modalPanel=document.getElementById("shiftfit-profile-modal-panel");
    if(modalPanel)return modalPanel;
    const direct=document.getElementById("profileScreen");
    if(direct&&getComputedStyle(direct).display!=="none")return direct;
  ...
  function boot(){styles();modal();window.shiftfitOpenDataPrivacy=open;inject();new MutationObserver(inject).observe(document.body,{subtree:true,childList:true});[300,800,1500,2500].forEach(function(t){setTimeout(inject,t);});}
