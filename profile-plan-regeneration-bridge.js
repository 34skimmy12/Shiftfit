...
(function(){
  ...
  function regenerateFromSavedTargets(){
    const plan=readPlan();
    if(!plan||typeof plan!=="object"){if(typeof window.showPlanBuilder==="function")window.showPlanBuilder();return false;}
    ...
    savePlan(plan);
    try{
      if(typeof window.shiftfitGenerateMealPlan==="function"){
        const generated=window.shiftfitGenerateMealPlan(plan);
        if(!generated)return false;
      }else if(typeof window.generateWeeklyMealPlan==="function"){
        window.generateWeeklyMealPlan(plan);
      }else{return false;}
      ...
      return true;
    }catch(error){console.warn("Profile plan regeneration failed.",error);if(typeof window.showPlanBuilder==="function")window.showPlanBuilder();return false;}
  }
  ...
  function installSignupEntry(){
    ...
    if(!style){style=document.createElement("style");style.id="shiftfit-signup-entry-style";style.textContent=".home-header{position:relative}.home-header .logo{position:absolute;left:50%;transform:translateX(-50%) skew(-5deg);white-space:nowrap}.home-header .signup-entry{width:auto;min-width:76px;height:38px;padding:0 12px;border:1px solid rgba(139,92,246,.75);border-radius:12px;background:rgba(36,22,78,.72);color:#f5f3ff;font-size:10px;font-weight:950;letter-spacing:.6px;box-shadow:0 0 14px rgba(109,40,217,.22);display:flex;align-items:center;justify-content:center}.home-header .signup-entry:active{transform:scale(.97)}";document.head.appendChild(style);}
    function apply(){
      var header=document.querySelector(".home-header");if(!header)return;
      var menu=header.querySelector(".menu-btn");
      if(menu){menu.className="signup-entry";menu.removeAttribute("aria-label");menu.innerHTML="<span>SIGN UP</span>";menu.type="button";if(menu.dataset.shiftfitSignupBound!=="1"){menu.dataset.shiftfitSignupBound="1";menu.addEventListener("click",function(event){event.preventDefault();event.stopPropagation();openSignup();});}}
      var existing=header.querySelector(".signup-entry");
      if(existing&&existing.dataset.shiftfitSignupBound!=="1"){existing.dataset.shiftfitSignupBound="1";existing.addEventListener("click",function(event){event.preventDefault();event.stopPropagation();openSignup();});}
    }
    apply();
