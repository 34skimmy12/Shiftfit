...
(function(){
  ...
  function regenerateFromSavedTargets(){
    ...
    savePlan(plan);
    try{if(typeof window.generateWeeklyMealPlan==="function")window.generateWeeklyMealPlan(plan);if(typeof window.updatePlan==="function")window.updatePlan(plan);if(typeof window.showPlanSuccess==="function")window.showPlanSuccess();if(typeof window.showMeals==="function")window.showMeals();}catch(error){console.warn("Profile plan regeneration failed.",error);if(typeof window.showPlanBuilder==="function")window.showPlanBuilder();}
  }
  function installBridge(){
    if(!document.body||document.getElementById("shiftfit-profile-plan-generate-bridge"))return;
    const button=document.createElement("button");button.id="shiftfit-profile-plan-generate-bridge";button.type="button";button.textContent="Save & Generate My Plan";button.style.cssText="position:fixed;left:-9999px;top:-9999px;width:1px;height:1px;opacity:0;pointer-events:none;";button.setAttribute("aria-hidden","true");button.addEventListener("click",function(event){event.preventDefault();event.stopPropagation();regenerateFromSavedTargets();});document.body.insertBefore(button,document.body.firstChild);
  }
