...
(function(){
  ...
  function regenerateFromSavedTargets(){const plan=readPlan();if(!plan||typeof plan!=="object"){if(typeof window.showPlanBuilder==="function")window.showPlanBuilder();return;}const fats=Number(plan.fats??plan.fat??plan.targetFats??plan.targetFat);if(Number.isFinite(fats)&&fats>0){plan.fats=fats;plan.fat=fats;plan.targetFats=fats;plan.targetFat=fats;}["calories","protein","carbs"].forEach(function(key){const value=Number(plan[key]??plan["target"+key.charAt(0).toUpperCase()+key.slice(1)]);if(Number.isFinite(value)&&value>0)plan[key]=value;});savePlan(plan);try{if(typeof window.generateWeeklyMealPlan==="function")window.generateWeeklyMealPlan(plan);if(typeof window.updatePlan==="function")window.updatePlan(plan);if(typeof window.showPlanSuccess==="function")window.showPlanSuccess();if(typeof window.showMeals==="function")window.showMeals();}catch(error){console.warn("Profile plan regeneration failed.",error);if(typeof window.showPlanBuilder==="function")window.showPlanBuilder();}}
  ...
  function boot(){installBridge();installAccountBackFix();signupStyles();installSignupTab();loadTheme();loadAuth();loadCloudSync();loadAccountSyncUi();}
